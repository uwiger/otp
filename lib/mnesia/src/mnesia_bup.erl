...
iterate(Mod, Fun, Opaque, Acc) ->
    ...
    case catch read_schema_section(R) of
        ...
        {R2, {Header, Schema, Rest}} ->
            ...
            case catch iter(R2, Header, Schema, Ext, Fun, Acc, Rest) of
                {ok, R3, Res} ->
                ...
                _ ->
                    case mnesia_schema:opt_create_dir(true, mnesia_lib:dir()) of
                        ...
                        ok ->
                            ...
                            file:delete(File),
                            case catch make_initial_backup(Ns, File, Mod, Props) of
                                {ok, _Res} ->
...
install_fallback_master(ClientPid, FA) ->
    ...
    Res = (catch iterate(Mod, fun restore_recs/5, Opaque, State)),
    unlink(ClientPid),
    ClientPid ! {self(), Res},
    exit(shutdown).
...
do_fallback_start(true, false) ->
    ...
    case catch iterate(Mod, fun restore_tables/5, BupFile, {start, LocalTabs}) of
        {ok, _Res} ->
