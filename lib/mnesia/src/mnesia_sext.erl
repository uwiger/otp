...
-export([prefix/1, enc_prefix/1,
	 partial_decode/1]).
-export([prefix_hex/1, enc_prefix_hex/1]).
-export([prefix_sb32/1, enc_prefix_sb32/1]).
...
enc_prefix_sb32(X) ->
    case enc_prefix(X) of
	{true , Enc} -> {true , chop_prefix_tail(to_sb32(Enc))};
	{false, Enc} -> {false, to_sb32(Enc)}
    end.
