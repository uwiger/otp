...
check_definition(TypeAlias, Tab, Nodes, Props) ->
    [Rc, Dc, DOc] =
        [proplists:get_value(K,[]) || K <- [ram_copies,
                                            disc_copies,
                                            disc_only_copies]],
    case {Rc,Dc,DOc} of
	{[],[],[]} ->
	    case prop(type, Props) of
                ordered_set -> ok;
                Type -> mnesia:abort({combine_error, Tab,
                                      [{TypeAlias, prop(TypeAlias, Props)},
                                       {type, Type}]})
            end,
	    NoMount = fun(Props) ->
			      mnesia:abort(
				{no_mount_point, Tab, Props})
		      end,
	    Props = prop(user_properties, Props),
	    case Props of
		[] ->
		    NoMount([]);
		[_|_] ->
	    ...
	    Attrs = prop(attributes, Props),
	    if hd(Attrs) =/= path ->
		    mnesia:abort({invalid_key, Tab, hd(Attrs)});
	       true ->
		    ok
	    end,
	    AttrMap = lists:map(
	    ...
	    Props1 = orddict:store(rofs_attr_map, AttrMap,
            ...
            lists:keyreplace(1, user_properties, Props,
                             {user_properties, Props1});
