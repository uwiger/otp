...
get_ext_modules_disc() ->
    lists:usort([ Module || {_,Module} <- get_ext_types_disc() ]).
...
check_keys_(Tab, [{Key, _Val} | Tail], Items) ->
    ...
    case lists:member(Key1, Items) of
        true ->  [Key | check_keys_(Tab, Tail, Items)];
        false -> mnesia:abort({badarg, Tab, Key})
    end;
check_keys_(_, [], _) ->
    [];
check_keys_(Tab, Arg, _) ->
...
real_suffixes() ->  [".DAT", ".LOG", ".BUP", ".DCL", ".DCD"] ++ ext_real_suffixes().

tmp_suffixes() -> [".TMP", ".BUPTMP", ".RET", ".DMP", "."] ++ ext_tmp_suffixes().

ext_real_suffixes() ->
    try Modules = get_ext_modules_disc(),
         lists:foldl(fun(Mod, Acc) -> Acc++Mod:real_suffixes() end, [], Modules)
    catch
        error:E ->
...
ext_tmp_suffixes() ->
    try Modules = get_ext_modules_disc(),
         lists:foldl(fun(Mod, Acc) -> Acc++Mod:tmp_suffixes() end, [], Modules)
    catch
        error:E ->
